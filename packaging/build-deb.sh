...
package_root="$(mktemp -d "${TMPDIR:-/tmp}/flexhr-deb.XXXXXX")"
trap 'rm -rf "$package_root"' EXIT
chmod 0755 "$package_root"

install -d -m 0755 "$package_root/DEBIAN" "$package_root/usr/bin"
install -m 0755 "$binary" "$package_root/usr/bin/flexhr"
...
Description: Flex HR command-line client
CONTROL

install -d -m 0755 "$output_dir"
dpkg-deb --build --root-owner-group \
  "$package_root" \
  "$output_dir/flexhr_${version}_${arch}.deb"
